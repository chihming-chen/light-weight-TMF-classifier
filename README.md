...
-	As shown below, the Support Vector Machine (SVM) model provides relatively low confidence scores even when it makes correct classifications. This puts the model in a disadvantage when participating in the 'soft' vote with uniform weights among all models in the Voting Classifier, especially when the SVM model outperforms the other models in Top-1 accuracy score. Exploring the weight distribution to balance the voting power should yield a better Voting Classifier model.
